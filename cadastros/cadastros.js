...
import { badgeHtml, escapeHtml, showMessage, attachCpfMask } from '../shared/utils.js';
...
export async function initCadastros(auth) {
  ...
  attachCpfMask(document.getElementById('funcCpf'));
  attachCpfMask(document.getElementById('editFuncCpf'));
...
function openFuncionarioModal(id) {
  ...
  document.getElementById('editFuncCpf').value = f.cpf || '';
...
async function handleFuncionarioEditSubmit(e) {
  ...
  const payload = {
    ...
    cpf: document.getElementById('editFuncCpf').value.trim() || null,
